...
/*******************************************************************************************
*spi_usr_type = 0 is analog spi, spi_usr_type = 1 is hardware spi.M25 do not support HW spi.
*******************************************************************************************/
u8 spi_usr_type = 0;
...
//spi device select¡ê?only for simulating SPI by software.
void spi_flash_cs(bool CS)
{
	if (!spi_usr_type)
	{
		if (CS)
			Ql_GPIO_SetLevel(PINNAME_PCM_SYNC,PINLEVEL_HIGH);
		else
			Ql_GPIO_SetLevel(PINNAME_PCM_SYNC,PINLEVEL_LOW);
	}
}
...
void proc_main_task(void)
{
    ...
    ret = Ql_SPI_Init(USR_SPI_CHANNAL,PINNAME_PCM_CLK,PINNAME_PCM_IN,PINNAME_PCM_OUT,PINNAME_PCM_SYNC,spi_usr_type);
