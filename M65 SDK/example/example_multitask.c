 ...
 *     If input "Ql_OS_SetLastErrorCode", that will learn how to set errorcode.
 *     If input "Ql_OS_GetLastErrorCode", that will get the current errorcode.
...
static void CallBack_UART_Hdlr(Enum_SerialPort port, Enum_UARTEventType msg, bool level, void* customizedPara)
{
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            else if(NULL != (p = Ql_strstr(pData, "Ql_OS_SetLastErrorCode")))
            {                                    //Ql_OS_SetLastErrorCode=ErrorCode
                p = Ql_strstr(p, "=");
                if(p == NULL)
                {
                    APP_DEBUG("\r\n<--Ql_OS_SetLastErrorCode: parameter ERROR!-->\r\n");
                    break;
                }

                p++;
                iNum = Ql_atoi(p);
                iRet = Ql_OS_SetLastErrorCode(iNum);
                APP_DEBUG("\r\n<--Ql_OS_SetLastErrorCode(%d)=%d-->\r\n", iNum, iRet);
                break;
            }
            else if(NULL != (p = Ql_strstr(pData, "Ql_OS_GetLastErrorCode")))
            {
                iNum = Ql_OS_GetLastErrorCode();
                APP_DEBUG("\r\n<--Ql_OS_GetLastErrorCode: ErrorCode=%d-->\r\n", iNum);
                break;
            }
