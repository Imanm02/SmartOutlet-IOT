...
static bool Dfota_Upgrade_States(Upgrade_State state, s32 fileDLPercent);
...
s32 Ql_DFOTA_StartUpgrade(u8* url, ST_GprsConfig* apnCfg, Callback_Upgrade_State callbcak_UpgradeState_Ind)
{
    ...
    if(NULL != callbcak_UpgradeState_Ind)
    {
        ...
        Fota_UpgardeState = Dfota_Upgrade_States;    // Use the default callback
    }
#ifdef __OCPU_DFOTA_BY_FTP__
    if(FTP_IsFtpServer(url))
    {
        FOTA_UPGRADE_IND(UP_START,0,retValue);
        ret = FTP_DfotaMain(contextId, url);
    }
    else
#endif
    ...
    {
        FOTA_UPGRADE_IND(UP_START,0,retValue);
        ret = HTTP_DfotaMain(contextId, url);
    }
    else
#endif
    {
