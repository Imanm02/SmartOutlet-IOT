...
s32 FTP_DfotaMain(u8 contextId, u8* URL)
{
    s32 retValue;
    bool ftpDecodeURL;

    FOTA_DBG_PRINT("Enter into FTP_DfotaMain()");
...
void FTP_Callback_OnDownload(s32 result,s32 size)
{
    ...
    // Start to do upgrading by DFOTA
    if (result)
    {
        DoUpgrade();
    }
...
static void FTP_Program(void)
{
	...
	UPGRADE_APP_DEBUG(FOTA_DBGBuffer, "<-- Set GPRS PDP context(%d), ret=%d -->\r\n", Contextid,ret);
    ...
    {
		ret = RIL_FTP_QFTPGET(appBin_fName, 0, FTP_Callback_OnDownload);//ignore size.
    }
    else
    {
...
static s32 DoUpgrade(void)
{
    ...
    if (QL_RET_OK == ret2)
    {
        ...
        while(filesize>0)
        {
            ...
            ret3 = Ql_DFOTA_WriteData((u8*)file_buffer,realLen);
            if(ret3 != 0)
            {
    ...
    Ql_Sleep(10);
	ret3 = Ql_DFOTA_Finish();     //Finish the upgrade operation ending with calling this API
    if(ret3 != 0)
    {
