...
#define TEMP_DBG_BUF_LEN 512
char Temp_DBGBuffer[TEMP_DBG_BUF_LEN];
                            ...
                            u8 *hostname, u16 hostnamelength, u16 *hostport)
{
    ...
    *hostport = HTTP_SERVICE_PORT;
    ...
    Ql_memset(Temp_DBGBuffer,0, TEMP_DBG_BUF_LEN);
    Ql_sprintf(Temp_DBGBuffer, "<-- hostname=%s -->\r\n", hostname);
    FOTA_DBG_PRINT(Temp_DBGBuffer);

    Ql_memset(Temp_DBGBuffer,0, TEMP_DBG_BUF_LEN);
    Ql_sprintf(Temp_DBGBuffer, "<-- hostip=%s -->\r\n", hostip);
    FOTA_DBG_PRINT(Temp_DBGBuffer);

    Ql_memset(Temp_DBGBuffer,0, TEMP_DBG_BUF_LEN);
    Ql_sprintf(Temp_DBGBuffer, "<-- hostport=%d -->\r\n", *hostport);
    FOTA_DBG_PRINT(Temp_DBGBuffer);

    Ql_memset(Temp_DBGBuffer,0, TEMP_DBG_BUF_LEN);
    Ql_sprintf(Temp_DBGBuffer, "<-- AddressValidLegth=%d, Address=%s -->\r\n", *AddressValidLegth, Address);
    FOTA_DBG_PRINT(Temp_DBGBuffer);
...
extern HttpMainContext_t DFOTA_HttpMainContext;
s32 http_DecodeHeader(HttpHeader_t *httpheader,  u8 *decode_buffer, s32 decode_buffer_length, s32 *decode_length)
{
    ...
    HttpMainContext_t     *httpContext_p = &DFOTA_HttpMainContext;
