...
bool HTTP_IsHttpServer(u8* URL)
{
    ...
    Ql_memcpy(buffer, URL, URL_HEADER_LENGTH);
...
s32 HTTP_DfotaMain(u8 contextId, u8* URL)
{
    ...
    FOTA_DBG_PRINT("Enter into HTTP_DfotaMain\r\n");
...
cuntinuerecvbodydata:
    ...
    if(httpContext_p->genhttp_dstpos)
    {
     ...
     //When the effective data length is all in the header package
    if(httpContext_p->genhttp_dstvaliddatalen >= httpContext_p->ContentLength)
    {
        Ql_SOC_Close(httpContext_p->socketid);
        httpContext_p->socketid = -1;
        httpContext_p->bpeersocketclose = TRUE;
        UPGRADE_APP_DEBUG(FOTA_DBGBuffer,"<-- bodydata received End (len=%d)-->\r\n",httpContext_p->ContentLength);
        //APP_DEBUG("<-- bodydata received End -->\r\n");
        FOTA_DBG_PRINT("<-- Downloading finished -->\r\n");
        Ql_Sleep(300);
        ret = Ql_DFOTA_WriteData((u8*)(httpContext_p->genhttp_dstconstptr+httpContext_p->genhttp_dstpos),httpContext_p->genhttp_dstvaliddatalen);
        if(NULL != httpContext_p->http_socketdata_p)
        {
            Ql_MEM_Free((void *)(httpContext_p->http_socketdata_p) );
            httpContext_p->http_socketdata_p = NULL;
        }
	     UPGRADE_APP_DEBUG(FOTA_DBGBuffer,"Ql_DFOTA_WriteData:%d\r\n", ret);
        if(ret !=0)
        {
    ...
    if(willrecvlength && (!httpContext_p->bpeersocketclose))
    {
		...
		//count += httpContext_p->genhttp_dstvaliddatalen;
    ...
    if(httpContext_p->receivedbodydata >= httpContext_p->ContentLength)
    {
		...
		ret = Ql_DFOTA_Finish();     //Finish the upgrade operation ending with calling this API
        if(ret !=0)
        {
            UPGRADE_APP_DEBUG(FOTA_DBGBuffer, "<--Ql_FOTA_Finish failed(ret =%d)-->\r\n",ret); 
            FOTA_DBG_PRINT("<-- Ql_FOTA_Finish failed -->\r\n");
