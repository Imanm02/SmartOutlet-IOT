...
typedef enum {
    ...
    VIRTUAL_PORT2,
    UART_START = 10,
    UART_PORT1 = UART_START,//main uart
    UART_PORT2,//aux uart
    UART_PORT_END
}Enum_SerialPort;
...
*                 This function is only effect on the physical port(UART_PORT1 UART_PORT2).
...
*               data bits, stop bits, parity,and flow control.
*               This function is only effect on the physical port(UART_PORT1 and UART_PORT2 ).
*                the virtual port(VIRTUAL_PORT1,VIRTUAL_PORT2) don't support this function.
*
* Parameters:
*               [in]port:
...
*                        the get option of UART_VFIFO is only valid on the UART_PORT1 and 
*                        the get option of UART_CLR_FE is only valid on the UART_PORT1 and UART_PORT2  
...
*                        the get option of UART_TX_BUF_SEND_OUT is only valid on the UART_PORT1 and UART_PORT2 
*              [out]para:
...
*                or clear the physical UART's(UART_PORT1 and UART_PORT2 ) frame error.
*
* Parameters:
*               [in]port:
*                         Port name
*               [in]opt:
*                        one value of Enum_UARTOption
*                        the set option of UART_VFIFO is only valid on the UART_PORT1 
*                        the set option of UART_CLR_FE is only valid on the UART_PORT1 and UART_PORT2
